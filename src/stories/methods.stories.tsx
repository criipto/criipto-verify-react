...
function LoginWithRedirectButton(props: any) {
  const { loginWithRedirect, isLoading, isInitializing, result } =
    useCriiptoVerify();
  useEffect(() => {
  ...
  return (
    <button onClick={() => loginWithRedirect({ acrValues: props.acrValues })}>
      loginWithRedirect
    </button>
  );
}
...
function LoginWithPopupButton(props: any) {
  ...
  return (
    <button
      onClick={() =>
        loginWithPopup({ acrValues: props.acrValues, backdrop: props.backdrop })
      }
    >
      loginWithPopup
    </button>
